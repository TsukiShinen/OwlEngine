...
namespace Owl
{
	struct Matrix4
	{
		...
		static Matrix4 Orthographic(const float pLeft, const float pRight, const float pBottom, const float pTop, const float pNearClip, const float pFarClip)
		{
		...
		static Matrix4 Perspective(const float pFovRadians, const float pAspectRatio, const float pNearClip, const float pFarClip) {
		...
		static Matrix4 LookAt(Vector3 pPosition, Vector3 pTarget, Vector3 pUp) {
			Matrix4 matrix;
			Vector3 zAxis { pTarget.x - pPosition.x,
