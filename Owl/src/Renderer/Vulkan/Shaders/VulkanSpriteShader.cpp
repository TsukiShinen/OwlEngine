...
namespace Owl
{
		...
		: m_Context(pContext)
	{
		...
		constexpr VkShaderStageFlagBits stageTypes[SPRITE_SHADER_STAGE_COUNT] = { VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT };

		for (uint32_t i = 0; i < SPRITE_SHADER_STAGE_COUNT; ++i)
		{
			if (!VulkanShaderUtils::CreateShaderModule(m_Context, BUILTIN_SHADER_NAME_SPRITE, stageTypesStrings[i], stageTypes[i], &m_Stages[i]))
			{
				OWL_CORE_CRITICAL("[VulkanSpriteShader] Unable to create %s shader module for '%s'.", stageTypesStrings[i], BUILTIN_SHADER_NAME_SPRITE);
				return;
			}
		}
