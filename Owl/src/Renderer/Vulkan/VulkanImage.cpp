...
namespace Owl
{
	VulkanImage::VulkanImage(VulkanContext* pContext, const uint32_t pWidth, const uint32_t pHeight, const VkFormat pFormat, const VkImageTiling pTiling,
	                         const VkImageUsageFlags pUsage, VkMemoryPropertyFlags pMemoryFlags, const bool pCreateView, const VkImageAspectFlags pAspectFlags)
			: m_Context(pContext), m_Width(pWidth), m_Height(pHeight)
	{
		...
		auto result = vkCreateImage(m_Context->Device->GetLogicalDevice(), &imageCreateInfo, m_Context->Allocator, &m_Handle);
		...
		result = vkAllocateMemory(m_Context->Device->GetLogicalDevice(), &memoryAllocateInfo, m_Context->Allocator, &m_Memory);
		OWL_CORE_ASSERT(result == VK_SUCCESS, "[VulkanImage] Failed to allocate memory!")

		result = vkBindImageMemory(m_Context->Device->GetLogicalDevice(), m_Handle, m_Memory, 0);  // TODO: configurable memory offset.
	...
	void VulkanImage::CreateView(const VkFormat pFormat, const VkImageAspectFlags pAspectFlags)
	{
		...
		const auto result = vkCreateImageView(m_Context->Device->GetLogicalDevice(), &viewCreateInfo, m_Context->Allocator, &m_View);
