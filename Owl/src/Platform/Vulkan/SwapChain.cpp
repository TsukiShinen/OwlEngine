...
{
	SwapChain::SwapChain(Device& pDevice, const VkExtent2D pWindowExtent)
		: m_Device(pDevice), m_WindowExtent(pWindowExtent)
	{
		Initialize();
	}
	...
	SwapChain::SwapChain(Device& pDevice, const VkExtent2D pWindowExtent,
		...
		: m_Device(pDevice), m_WindowExtent(pWindowExtent), m_OldSwapChain(pPrevious)
	{
		Initialize();
	...
	VkResult SwapChain::AcquireNextImage(uint32_t* pImageIndex) const
	{
		vkWaitForFences(
			m_Device.GetDevice(),
			1,
	...
	VkResult SwapChain::SubmitCommandBuffers(const VkCommandBuffer* pBuffer, const uint32_t* pImageIndex)
	{
		if (m_ImagesInFlight[*pImageIndex] != VK_NULL_HANDLE)
		{
			vkWaitForFences(m_Device.GetDevice(), 1, &m_ImagesInFlight[*pImageIndex], VK_TRUE, UINT64_MAX);
		}
	...
	{
		OWL_CORE_INFO("===== Create Swap chain");
	...
	VkSurfaceFormatKHR SwapChain::ChooseSwapSurfaceFormat(
		const std::vector<VkSurfaceFormatKHR>& pAvailableFormats)
	{
	...
	VkPresentModeKHR SwapChain::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& pAvailablePresentMode)
	{
		for (const auto& availablePresentMode : pAvailablePresentMode)
		{
			if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR)
			{
				OWL_CORE_INFO("Present mode: Mailbox");
		...
		// 	{
		// 		OWL_CORE_INFO("Present mode: Immediate");
		// 		return availablePresentMode;
		// 	}
		// }

		OWL_CORE_INFO("Present mode: V-Sync");
		return VK_PRESENT_MODE_FIFO_KHR;
	}

	VkExtent2D SwapChain::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& pCapabilities) const
	{
		...
		actualExtent.width = std::max(
			...
			std::min(pCapabilities.maxImageExtent.width, actualExtent.width));
		actualExtent.height = std::max(
			pCapabilities.minImageExtent.height,
			std::min(pCapabilities.maxImageExtent.height, actualExtent.height));
